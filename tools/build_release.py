...
PLUGIN_ROOT = PACKAGE_ROOT / "mine_resilience_qgis"
OUTPUT_ROOT = PACKAGE_ROOT / "dist"
INSTALL_ZIP = OUTPUT_ROOT / "Mine-Water-Infrastructure-Resilience-QGIS-1.0.zip"
SOURCE_ZIP = OUTPUT_ROOT / "Mine-Water-Infrastructure-Resilience-QGIS-1.0-Source.zip"
...
def validate_install_zip(path: Path) -> None:
    with zipfile.ZipFile(path) as archive:
        ...
        required = {
            "mine_resilience_qgis/__init__.py",
            "mine_resilience_qgis/metadata.txt",
            "mine_resilience_qgis/plugin.py",
            "mine_resilience_qgis/provider.py",
            "mine_resilience_qgis/algorithms/preflight.py",
            "mine_resilience_qgis/algorithms/fully_hydro.py",
            "mine_resilience_qgis/algorithms/mine_resilience.py",
            "mine_resilience_qgis/mining_core.py",
            "mine_resilience_qgis/docs/README.md",
            "mine_resilience_qgis/docs/QUICK_START.md",
        }
        missing = sorted(required - names)
        unwanted = sorted(name for name in names if "__pycache__" in name or name.endswith(".pyc"))
        roots = {name.split("/", 1)[0] for name in names}
        if missing or unwanted or roots != {"mine_resilience_qgis"}:
            raise RuntimeError(
                f"Invalid install ZIP: missing={missing}, unwanted={unwanted}, roots={sorted(roots)}"
            )
        metadata = archive.read("mine_resilience_qgis/metadata.txt").decode("utf-8")
        for item in ("qgisMinimumVersion=3.44", "hasProcessingProvider=yes", "version=1.0.0"):
...
def main() -> None:
    ...
    (OUTPUT_ROOT / "MINE_RESILIENCE_RELEASE_CHECKSUMS.json").write_text(
